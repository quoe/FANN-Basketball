...
namespace FANN_Library
{
    public class FANNClass
    {
        DataType[] calc_out;
        uint num_layers = 3;
        uint num_input = 2;
        ...
        public void SetNetTrainParams(ActivationFunction ActivationFunctionHidden, ActivationFunction ActivationFunctionOutput, StopFunction TrainStopFunction, double BitFailLimit, TrainingAlgorithm TrainingAlgorithm)
        {
            ...
            this.ScaleNewInputMin = 0;
            this.ScaleNewInputMax = 1;
            this.ScaleNewOutputMin = 0;
            this.ScaleNewOutputMax = 1;
        }
        ...
        public float TrainOnIO(double[] input, double[] output, int trainCount, bool ScaleInput, bool DefaultTrainParams)
        {
            ...
            return TrainOnDataEpoch(TrainData, trainCount, ScaleInput, DefaultTrainParams);
        }

        public float TrainOnIO(float[] input, float[] output, int trainCount, bool ScaleInput, bool DefaultTrainParams)
        {
            double[] doubleInput = new double[input.Length];
            for (int i = 0; i < doubleInput.Length; i++)
            {
                doubleInput[i] = (double)input[i];
            }
            double[] doubleOutput = new double[output.Length];
            for (int i = 0; i < doubleOutput.Length; i++)
            {
                doubleOutput[i] = (double)output[i];
            }
            return TrainOnIO(doubleInput, doubleOutput, trainCount, ScaleInput, DefaultTrainParams);
        }
